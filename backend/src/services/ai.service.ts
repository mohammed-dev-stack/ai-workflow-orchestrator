...
// ============================================================
// خدمة الذكاء الاصطناعي (AI Service) – تدير استدعاءات Claude API
// وتوليد القرارات والرسائل الإلكترونية مع دعم وضع المحاكاة (mock).
// ✅ تم إصلاح الوصول إلى إعدادات aiConfig عبر anthropic.
// ============================================================
...
export class AIService {
  ...
  static getStatus(): {
    ...
    return {
      ...
      model: aiConfig.anthropic.model || DEFAULT_MODEL, // ✅ إصلاح
    };
  }
  ...
  private static async generateRealDecision(
    ...
    const response = await client.messages.create({
      model: aiConfig.anthropic.model || DEFAULT_MODEL, // ✅ إصلاح
      max_tokens: aiConfig.anthropic.maxTokens || DEFAULT_MAX_TOKENS, // ✅ إصلاح
      temperature: aiConfig.anthropic.temperature || DEFAULT_TEMPERATURE, // ✅ إصلاح
      system: systemPrompt,
      messages: [
        {
          role: 'user',
